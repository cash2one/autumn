...
class Dell(threading.Thread):
    ...
    def run(self):
        ...
        while len(taglist) > 0:
            ...
            mutex.release()
            #Get Dell's page
            url = "http://www.dell.com/support/home/cn/zh/cndhs1/product-support/servicetag/" + \
            ...
            try:
                server_no_with_tags = soup.find(attrs={'id': 'ProductExpressServiceTag'}).text
                server_no = re.compile(r'\d+').findall(server_no_with_tags)[0]
                print 'No:', server_no
            ...
            tagdict[server_no] = date
            taglistkey = list(taglistkey)
            index = taglistkey.index(self.tag)
            taglistkey.remove(self.tag)
            taglistkey.insert(index, server_no)
            taglistkey = tuple(taglistkey)
