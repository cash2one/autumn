...
class BaseDownloadHtml(object):
    ...
    def get_html(self, url, data=None, method='GET', headers=None, encoding=False):
        ...
        """you can use many methods to base web source, here use requests."""
    ...
    def downloader_browser(self, url, data=None, headers=None, encoding=False):
        """
            ...
            sp: if need, can sleep(3) after base web,general no need
...
if __name__ == '__main__':
    ...
    g_url = 'http://www.mofcom.gov.cn/article/ae/ai/201511/20151101192504.shtml'
    taobao_headers = {'referer': 'https://detail.tWWmall.com/item.htm?spm=a220m.1000858.1000725.1.Fn7NlQ&id=36424250160&skuId=77598565279&areaId=810000&cat_id=50025135&rn=dd660f9a7ceea4e4235a8bfd76599d0b&user_id=408107205&is_b=1'}

    # html = BaseDownloadHtml().get_html(g_url)[0]
    # html = BaseDownloadHtml().downloader_browser(g_url, encoding=True)
    # print html
    # document = PyQuery(html)
    # print document('#J_AttrUL').text()

    url = 'http://query.sse.com.cn/marketdata/tradedata/queryMargin.do?jsonCallBack=&isPagination=true' \
          '&tabType=mxtype&detailsDate=20160125&stockCode=&beginDate=&endDate=&pageHelp.pageSize=15&' \
          'pageHelp.pageCount=50&pageHelp.pageNo=3&pageHelp.beginPage=10&pageHelp.cacheSize=1&pageHelp.endPage=21&'

    headers = {
       # 'Cookie': 'PHPStat_First_Time_10000017=1451899958169; PHPStat_Cookie_Global_User_Id=_ck16010417323812216057018165977; PHPStat_Return_Time_10000017=1451899958169; PHPStat_Msrc_10000017=%3A%3Amarket_type_free_search%3A%3A%3A%3Abaidu%3A%3A%3A%3A%3A%3A%3A%3Awww.baidu.com%3A%3A%3A%3Apmf_from_free_search; PHPStat_Msrc_Type_10000017=pmf_from_free_search; PHPStat_Msrc_First_10000017=%3A%3Amarket_type_free_search%3A%3A%3A%3Abaidu%3A%3A%3A%3A%3A%3A%3A%3Awww.baidu.com%3A%3A%3A%3Apmf_from_free_search; PHPStat_Main_Website_10000017=_ck16010417323812216057018165977%7C10000017%7C%7C%7C; PHPStat_First_Time_10000011=1453797591241; PHPStat_Return_Time_10000011=1453797591241; PHPStat_Main_Website_10000011=_ck16010417323812216057018165977%7C10000011%7C%7C%7C',
       'Referer': 'http://www.sse.com.cn/services/tradingservice/margin/info/securities/'
    }

    print BaseDownloadHtml().get_html(url, headers=headers)[0]
